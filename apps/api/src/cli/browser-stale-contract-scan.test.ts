...
describe("stale contract rules", () => {
  ...
  test("requires one exported atomic adapter activation", () => {
    const bad = [
      {
        path: "x.ts",
        text: "function activateAdapterRun() {} function activateAdapterCapability() {}",
      },
    ];
    expect(
      ruleIds(bad).filter(id => id === "split_adapter_activation").length,
    ).toBeGreaterThanOrEqual(3);
    expect(
      ruleIds([
        {
          path: "x.ts",
          text: "export function activateAdapterProcess() {}",
        },
      ]),
    ).not.toContain("split_adapter_activation");
    expect(
      ruleIds([
        {
          path: "first.ts",
        ...
        {
          path: "second.ts",
          text: "export const activateAdapterProcess = () => {};",
        },
      ]),
    ).toContain("split_adapter_activation");
  });

  test.each([
    [
      "exact",
      "export const codeRunResultSchema = z.strictObject({ stdout: x, result: x, stderr: x, exitCode: x, killed: x }); export type CodeRunResult = z.infer<typeof codeRunResultSchema>;",
      false,
    ],
    [
      "fromCache",
      "export const codeRunResultSchema = z.strictObject({ stdout: x, result: x, stderr: x, exitCode: x, killed: x, fromCache: x }); export type CodeRunResult = z.infer<typeof codeRunResultSchema>;",
      true,
    ],
    [
      "missing killed",
      "export const codeRunResultSchema = z.strictObject({ stdout: x, result: x, stderr: x, exitCode: x }); export type CodeRunResult = z.infer<typeof codeRunResultSchema>;",
      true,
    ],
    [
      "wrong alias",
      "export const codeRunResultSchema = z.strictObject({ stdout: x, result: x, stderr: x, exitCode: x, killed: x }); export type CodeRunResult = z.infer<typeof otherSchema>;",
      true,
    ],
  ])("checks code result: %s", (_name, text, bad) => {
    const sources = [
      {
        path: "apps/api/src/lib/browser-runtime/protocol.ts",
        text,
      },
      {
        path: "activation.ts",
    ...
    expect(ruleIds(sources).includes("stale_code_result")).toBe(bad);
  });
});
