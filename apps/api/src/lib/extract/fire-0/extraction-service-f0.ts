...
export async function performExtraction_F0(
  ...
  if ((!request.urls || request.urls.length === 0) && request.prompt) {
    ...
    const rephrasedPrompt = await generateBasicCompletion_FO(
  ...
  if (isMultiEntity && reqSchema) {
    ...
    for (const chunk of chunks) {
      const chunkPromises = chunk.map(async doc => {
        try {
          ...
          const completionPromise = batchExtractPromise_F0(
            multiEntitySchema,
            links,
            request.prompt ?? "",
    ...
    try {
      ...
      sourceTracker.transformResults_F0(
        extractionResults,
        multiEntitySchema,
        false,
      );
      ...
      const multiEntitySources = sourceTracker.mapSourcesToFinalItems_F0(
        multiEntityResult,
        multiEntityKeys,
      );
  ...
  if (
    ...
    } = await singleAnswerCompletion_F0({
      singleAnswerDocs,
      rSchema,
      links,
      prompt: request.prompt ?? "",
