...
export function getModel(name: string, provider: Provider = defaultProvider) {
  ...
  // o3-mini returns empty text via the Responses API. Chat-only OpenAI-compatible
  // endpoints also need every language model routed through Chat Completions.
  if (
    provider === "openai" &&
    (config.OPENAI_CHAT_COMPLETIONS_ONLY || modelName.startsWith("o3-mini"))
  ) {
