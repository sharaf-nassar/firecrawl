...
export async function runLifecycleSelfTest({ silent = false } = {}) {
  ...
  const stopFixture = async ({ child, deadline, registry }) => {
    let closed = false;
    ...
    try {
      gracefulWaitMs = Math.min(1_000, deadline.remaining());
    } catch {
      gracefulWaitMs = 0;
    }
    if (!closed && gracefulWaitMs > 0) {
      await Promise.race([
        closedPromise.then(() => true),
        wait(gracefulWaitMs).then(() => false),
      ]);
    }
    await registry.terminateProcessGroup(child.pid, { graceful: closed });
    if (!closed) {
      await registry.withDeadline(
        closedPromise,
        registry.now() + CLEANUP_DRAIN_GRACE_MS,
  ...
  const appDeadline = new ProcessDeadline(1, () => operationalNow);
  appRegistry.ownProcessGroup(appChild);
  operationalNow = 2;
  queueMicrotask(() => {
    appChild.stdout.end();
    appChild.stderr.end();
  ...
  drainRegistry.ownProcessGroup(drainChild);
  await assert.rejects(
    stopFixture({
      child: drainChild,
