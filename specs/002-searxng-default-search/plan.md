...
Bundle one private SearXNG service only in `compose.local.yaml`, making it the bundled provider for `scripts/local-firecrawl` after required Brave credential setup. Endpoint and engine selection remain automatic. Keep generic Compose, Helm, and hosted deployments provider-explicit. Direct DuckDuckGo code is removed globally, and any installation with no configured provider receives a typed unavailable error.
...
Track `config/searxng/settings.yml` with JSON output, POST form method, blank autocomplete/favicon resolver, private/no-limiter/no-image-proxy behavior, no Valkey, default category `general`, and an explicit `keep_only` engine set: official `braveapi` and `bing`. The tracked Brave entry remains inactive and contains no credential; the private launcher injects the required key and activates only that exact entry in tmpfs before startup. A boot test must inspect SearXNG's effective settings and prove both engines are enabled before any functional smoke is accepted. Disable every DuckDuckGo path by construction. Use safe search level 1, default language `en`, maximum page 5, engine request timeout 3 seconds, maximum engine timeout 4 seconds, zero retries, `pool_connections: 16`, and `pool_maxsize: 8`.

Bundled mode requires the exact `braveapi,bing` engine list and a nonblank valid Brave credential before Docker starts; Bing-only operation and bundled engine overrides fail closed. Categories must be implemented by those engines, and any DuckDuckGo engine, alias, autocomplete provider, favicon resolver, or category expansion is rejected. A separately operated external SearXNG endpoint suppresses the bundled service, but the shared client still validates every explicit engine override against `braveapi,bing` and bounds categories to Firecrawl's supported contract. Firecrawl never selects or falls back to DuckDuckGo itself.

Harden the service with no host port, `backend` network only, UID/GID `977:977`, read-only root, dropped capabilities, `no-new-privileges`, init, rotating logs, read-only settings, `cpus: 1.0`, 512 MiB memory and swap limits, `pids_limit: 128`, and tmpfs limits of 64 MiB for `/tmp` and 128 MiB for `/var/cache/searxng`. Generate a dedicated 64-character lowercase hexadecimal `SEARXNG_SECRET`; do not expose it to API. Persist only the Base64-encoded Brave transport value in the mode-`0600` local environment, pass it only to SearXNG, and render the decoded key into a mode-`0600` tmpfs settings file.
...
- `.env.example.local`, `scripts/init-local-env.sh`, `scripts/upgrade-local-env-phase1`: default/migrate endpoint, privately require and encode the Brave key, generate and validate the service secret, preserve external overrides, and retain atomic/idempotent update guarantees.
- `scripts/local-firecrawl`: provider-mode detection from rendered configuration; mandatory bundled credential validation and rotation; conditional service inventory; start/stop/restart/status/health/logs; port and Compose invariants; redacted functional smoke.
...
- `SEARXNG_BRAVE_API_KEY_B64`: required Base64 transport encoding of the operator-supplied Brave key. Interactive setup and `configure-search` hide input; noninteractive setup reads the raw key only from process environment. The value is passed only to SearXNG and decoded only into tmpfs.
- `SEARXNG_ENGINES`: exactly `braveapi,bing` in bundled mode. Missing, reordered, reduced, or expanded values fail closed; external endpoints may explicitly select only a normalized subset of the same qualified pair.
- `SEARXNG_CATEGORIES`: optional category selection remains constrained to Firecrawl's supported contract and categories implemented by selected engines for both internal and external endpoints.
...
- Environment tests cover required-key setup, blank/whitespace rejection, credential rotation, missing/blank migration, external override preservation, secret generation/validation/uniqueness, invalid URL rejection, idempotency, modes, symlinks, locks, and concurrent changes.
...
- Qualify official `braveapi` and Bing on the supported local host baseline with three isolated smoke attempts each: at least two per engine must return one valid HTTP(S) result within the 4-second engine ceiling, without account cookies, CAPTCHA, or a changed outbound hostname. Supply the Brave credential without recording it. Both engines must qualify; never fall back to Bing alone or substitute DuckDuckGo.
...
### 2026-08-01 credentialed live qualification

The frozen engine and capacity values passed a bounded live acceptance run without recording queries, result content, URLs, or credential values.

- Official `braveapi` returned valid HTTP(S) results in 3/3 isolated attempts at 668, 240, and 305 ms. Bing returned valid results in 3/3 attempts at 240, 236, and 214 ms. No auth, rate-limit, CAPTCHA, account-cookie, hostname-drift, or upstream-error signal occurred.
- The Linux x86_64 host and Docker reported 64 logical CPUs and 125.6 GiB RAM. SearXNG retained its frozen 1 CPU, 512 MiB memory/swap, 128 PID, 64/128 MiB tmpfs, and 16/8 HTTP pool limits.
- After wrapper restart and full health passed, 20 sequential `/v2/search` requests had p50 497 ms, p95 880 ms, and max 896 ms. One batch of 8 concurrent requests had p50 549 ms, p95 1,961 ms, and max 1,961 ms. Overall p50 was 521 ms, p95 1,048 ms, and max 1,961 ms.
- All 28 Firecrawl requests returned structurally valid HTTP 200 web results with limit 5 and no scrape options. API and SearXNG remained healthy with zero restarts and no OOM; SearXNG used 108.2 MiB and 14 PIDs at final inspection.
...
- Upstream engines can still throttle or block one egress IP. Mitigation: official Brave API plus Bing diversity, explicit engine failures, bounded smoke, sanitized diagnostics, and benchmark before allowlist freeze; no SLA claim.
...
|---|---:|---|---|
| Implement secure local SearXNG service and settings | P1 | None | Exact image digest and manifest list, required Brave credential with fail-closed injection, explicit `braveapi,bing` config, effective-settings boot verification, JSON/POST, private network/no port, hardening/resources, readiness, and rendered/static tests pass. |
...
| Migrate local SearXNG environment | P1 | Secure service/settings | New and upgraded envs receive canonical endpoint/service secret safely; bundled setup requires and isolates the Brave credential; exact URL normalization, external overrides, and atomic/idempotent safeguards pass. |
...
| Qualify engines, benchmark, and freeze operational values | P1 | Service/settings, shared provider, environment, ordinary REST, internal consumers, local REST, lifecycle, MCP | Official `braveapi` and Bing both qualify; benchmark thresholds pass; final engine/resource/deadline/pool values are frozen in implementation and plan before downstream work. |
